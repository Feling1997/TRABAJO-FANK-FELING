...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if (!$errores) {
    ...
    ");
    if ($stmt) {
      $stmt->bind_param("sssssii", $nombre, $email, $tel, $dni, $direccion, $estado, $id);
      $stmt->execute();
      $stmt->close();
      header("Location: index.php?ok=1");
      exit;
    } else {
      $errores[] = "Error preparando la actualización: " . $conexion->error;
    }
  } else {
    $u = array_merge($u, [
      ...
      <div class="col-lg-8">
        <div class="card shadow-sm">
          <div class="card-header bg-white">
            <h1 class="h4 m-0">Editar usuario #<?= htmlspecialchars($u['id']) ?></h1>
          </div>
          <div class="card-body">
            <?php if ($errores): ?>
            ...
            <form method="post" class="row g-3">
              ...
              <div class="col-md-6">
                ...
                <input name="dni" class="form-control" value="<?= htmlspecialchars($u['dni']) ?>" required>
              </div>
              <div class="col-md-6">
                <label class="form-label">Dirección</label>
                <input name="direccion" class="form-control" value="<?= htmlspecialchars($u['direccion']) ?>">
              </div>
              <div class="col-md-6">
                <label class="form-label">Estado</label>
                <select name="estado" class="form-select">
                  <option value="1" <?= ((string)$u['estado']==='1')?'selected':''; ?>>Activo</option>
                  <option value="0" <?= ((string)$u['estado']!=='1')?'selected':''; ?>>Inactivo</option>
                </select>
              </div>
